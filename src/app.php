...
$app->get('/create-table', function (Silex\Application $app) {
	$file = fopen(__DIR__ . '/../data/schema2019.sql', 'r');
...
// Tipos
/***************************************************/
$app->get('/tipos/list', function () {
	ob_start();
	include __DIR__ . '/../templates/tipos/list.php';
	return ob_get_clean();
});

$app->get('api/tipos', function (Request $request) use ($app) {
	/** @var \Doctrine\DBAL\Connection $db */
	$db = $app['db'];
...
$app->post('api/tipos/store', function (Request $request) use ($app) {
	/** @var \Doctrine\DBAL\Connection $db */
	$db = $app['db'];
	$data = $request->request->all();
	$db->insert('tipos', $data);
	$id = $db->lastInsertId();
	$row = $db->fetchAssoc("SELECT * FROM tipos WHERE id = ?", [$id]);
	return $app->json($row);

});

$app->post('api/tipos/update', function (Request $request) use ($app) {
	/** @var \Doctrine\DBAL\Connection $db */
	$db = $app['db'];
	$data = $request->request->all();
	$id = $data['id'];
	unset($data['id']);
	$db->update('tipos', $data, ['id' => $id]);
	$row = $db->fetchAssoc("SELECT * FROM tipos WHERE id = ?", [$id]);
	return $app->json($row);
});

$app->post('api/tipos/delete', function (Request $request) use ($app) {
	/** @var \Doctrine\DBAL\Connection $db */
	$db = $app['db'];
	$data = $request->request->all();
	$id = $data['id'];
	$result = $db->delete('tipos', ['id' => $id]);
	return $app->json(['success' => $result != 0], $result != 0 ? 200 : 400);
});
